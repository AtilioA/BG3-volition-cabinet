...
VCHelpers.Dice = _Class:Create("HelperDice", Helper)
...
---@return integer
function VCHelpers.Dice:Roll(diceAmount, faces, minDieValue, maxDieValue)
    local total = 0
...
---@return integer
function VCHelpers.Dice:RollUpgradingDice(diceAmount, faces, startingDieValue, endingDieValue)
    ...
    while roll <= diceAmount do
        local minDie = startingDieValue +
        Ext.Math.Round((endingDieValue - startingDieValue) / (diceAmount - 1) * (roll - 1))
        total = total + VCHelpers.Dice:Roll(1, faces, minDie)
        roll = roll + 1
    end
    return total
end
